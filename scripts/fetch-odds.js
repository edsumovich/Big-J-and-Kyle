#!/usr/bin/env node
/**
 * Fetches NFL + NCAAF odds from ESPN's public scoreboard API and writes
 * them to data/games.json in the schema Slate's UI expects. Meant to be run
 * on a schedule (see .github/workflows/update-odds.yml).
 *
...
function toGame(event, sportLabel) {
  ...
  if (oddsObj) {
    // oddsObj.spread is a clean number: the HOME team's line (negative =
    // home favored, positive = home underdog) — no string parsing needed.
    if (typeof oddsObj.spread === "number") {
      spreadLine = Math.abs(oddsObj.spread);
      favorite = oddsObj.spread <= 0 ? "home" : "away";
    }
    ...
    // oddsObj.overUnder is likewise a clean number, no "o61.5"-style
    // string parsing needed.
    if (typeof oddsObj.overUnder === "number") totalLine = oddsObj.overUnder;
