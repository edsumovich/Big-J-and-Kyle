...
async function supabaseDelete(table, id) {
  ...
  if (!res.ok) throw new Error(`Supabase DELETE ${table}/${id} failed: ${res.status} ${await res.text()}`);
}
...
// radar_picks were never locked in and don't carry a result column at all
// — this doesn't grade them, it just removes ones that would clearly have
// lost, since there's no reason to keep showing a dead idea on someone's
// radar. Wins/pushes are deliberately left alone.
async function clearLostRadarPicks(eventsById) {
  const rows = await supabaseGet(`radar_picks?select=id,game_id,market,pick`);
  console.log(`radar_picks: ${rows.length} row(s) to check for cleanup`);

  let cleared = 0;
  for (const row of rows) {
    const entry = eventsById[row.game_id];
    if (!entry) continue; // game hasn't finished yet
    ...
    await supabaseDelete("radar_picks", row.id);
    cleared++;
    console.log(`Cleared lost radar pick ${row.id}: "${row.pick}" (${row.market})`);
  }
  console.log(`radar_picks: cleared ${cleared} of ${rows.length} row(s)`);
}
...
async function main() {
  ...
  await clearLostRadarPicks(eventsById);
}
