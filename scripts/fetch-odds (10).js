...
async function fetchOddsRows(league) {
  ...
  //
  // Pagination: a live response showed meta as a completely empty object
  // ({}), so meta.pagination.has_more (from the docs example) doesn't
  // actually exist in practice — relying on it made the loop stop after
  // page 1 every time, regardless of how much more data existed. Instead,
  // this just keeps requesting pages as long as a full page came back
  // (pageRows.length === limit), stopping once a page comes back
  // partially-full or empty, which works regardless of what (if anything)
  // the API puts in meta.
  const rows = [];
  let offset = 0;
  const limit = 100;
  let page = 0;
  const MAX_PAGES = 30; // safety cap — 30 * 100 = up to 3,000 rows per league
  ...
  do {
    ...
    if (process.env.LOG_RAW) {
      console.log(`${league} page ${page}: got ${pageRows.length} rows, meta:`, JSON.stringify(json.meta || {}));
      if (page === 0) {
    ...
    page++;
    if (pageRows.length < limit) break; // a partial (or empty) page means we've reached the end
    offset += limit;
  ...
  console.log(`${league}: fetched ${rows.length} odds row(s) across ${page} page(s), ${uniqueEvents} unique event(s), ${propCount} player props`);
  return rows;
}
